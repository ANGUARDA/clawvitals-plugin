...
Scheduled scans are **off by default**. The plugin will not run autonomously or add any cron entries until you explicitly enable a schedule.

To enable, use the `clawvitals_set_schedule` agent tool, or say something like "set clawvitals schedule to daily" in natural language. The default cron expression when enabled is 9 AM daily (`0 9 * * *`).
