...
export class ControlEvaluator {
    library;
    exclusions;
    constructor(library, exclusions) {
     ...
     */
    evaluate(collected) {
        return this.library.controls.map(control => this.evaluateControl(control, collected));
    }
    /**
     ...
     */
    evaluateControl(control, collected) {
     ...
     */
    evaluateStable(control, collected, base) {
        switch (control.id) {
            case 'NC-OC-003':
     ...
     */
    evaluateExperimental(_control, _collected, base) {
        return {
            ...base,
            result: 'SKIP',
    ...
    evalNCOC003(collected, base) {
        ...
        const found = this.hasFinding(collected.security_audit.data.findings, 'gateway.nodes.deny_commands_ineffective');
        return found
    ...
    evalNCOC004(collected, base) {
        ...
        if (openElevated || openRuntime) {
            const checkIds = [];
            if (openElevated)
                checkIds.push('security.exposure.open_groups_with_elevated');
            if (openRuntime)
                checkIds.push('security.exposure.open_groups_with_runtime_or_fs');
            return {
                ...base,
                result: 'FAIL',
    ...
    evalNCAUTH001(collected, base) {
        ...
        const found = this.hasFinding(collected.security_audit.data.findings, 'gateway.trusted_proxies_missing');
        return found
    ...
    evalNCOC012(collected, base) {
        ...
        const found = this.hasFinding(collected.security_audit.data.findings, 'gateway.loopback_no_auth');
        return found
    ...
    evalNCOC013(collected, base) {
        ...
        const found = this.hasFinding(collected.security_audit.data.findings, 'browser.control_no_auth');
        return found
    ...
    evalNCOC014(collected, base) {
        ...
        const found = this.hasFinding(collected.security_audit.data.findings, 'gateway.token_too_short');
        return found
     ...
     */
    computeVersionDistance(current, latest) {
