...
export class ExpandedEvaluator {
    ...
    evalDocker(r) {
        ...
        const risky = r.docker.containers.filter(c => c.privileged || c.root_user || c.dangerous_caps.length > 0);
        if (risky.length > 0) {
            const details = risky.map(c => {
                ...
                if (c.root_user)
                    issues.push('root');
                if (c.dangerous_caps.length > 0)
                    issues.push(`caps: ${c.dangerous_caps.join(',')}`);
...
//# sourceMappingURL=expanded-evaluator.js.map