 ...
 */
export function formatDetail(report, delta) {
    ...
    const critHigh = stable.findings.filter(f => f.result === 'FAIL' && (f.severity === 'critical' || f.severity === 'high'));
    ...
    // Medium findings
    const medium = stable.findings.filter(f => f.result === 'FAIL' && f.severity === 'medium');
    if (medium.length > 0) {
    ...
    // Low/Info findings
    const lowInfo = stable.findings.filter(f => f.result === 'FAIL' && (f.severity === 'low' || f.severity === 'info'));
    if (lowInfo.length > 0) {
...
function formatFinding(lines, finding) {
