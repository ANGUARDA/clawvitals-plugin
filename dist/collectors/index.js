...
export class CollectorOrchestrator {
     ...
     */
    async collect() {
        const [securityResult, healthResult, updateResult, versionResult] = await Promise.allSettled([
        ...
        const security_audit = securityResult.status === 'fulfilled'
            ...
            : { ok: false, data: null, ts: null, error: securityResult.reason instanceof Error ? securityResult.reason.message : 'Unknown error' };
        const health = healthResult.status === 'fulfilled'
            ? healthResult.value
            : { ok: false, data: null, ts: null, error: healthResult.reason instanceof Error ? healthResult.reason.message : 'Unknown error' };
        const update_status = updateResult.status === 'fulfilled'
            ? updateResult.value
            : { ok: false, data: null, ts: null, error: updateResult.reason instanceof Error ? updateResult.reason.message : 'Unknown error' };
        const version_cmd = versionResult.status === 'fulfilled'
            ? versionResult.value
            : { ok: false, version: null, error: versionResult.reason instanceof Error ? versionResult.reason.message : 'Unknown error' };
        ...
        if (security_audit.ok && security_audit.data) {
            const attackSurfaceFinding = security_audit.data.findings.find(f => f.checkId === 'summary.attack_surface');
